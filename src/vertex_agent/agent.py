...
class Agent:
    ...
    def __init__(
        ...
        if use_router and router_projects:
            self.router = VertexRotationManager(
                projects_config=router_projects,
    ...
    def add_router_project(self, project_config: Dict[str, Any]) -> None:
        """Add a new project to the router (only works when router is enabled)."""
        if not self.use_router:
            raise ValueError("Router is not enabled. Initialize agent with use_router=True.")
        self.router.add_project(project_config)
    
    def remove_router_project(self, project_id: str) -> None:
        """Remove a project from router (only works when router is enabled)."""
        if not self.use_router:
            raise ValueError("Router is not enabled. Initialize agent with use_router=True.")
        self.router.remove_project(project_id)
    ...
    def reset_daily_quotas(self) -> None:
        """Reset daily quotas for all projects (router only)."""
        if not self.use_router:
            raise ValueError("Router is not enabled. Initialize agent with use_router=True.")
        self.router.reset_daily_quotas()
    ...
    def _call_api(self, payload: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Internal method to call the appropriate API (direct or through router)."""
        if self.use_router:
            return self.router.call_gemini_api(payload, max_retries=3)
        else:
            return self.api_client.call_gemini_api(payload, config)
    ...
    def prompt(
        ...
        while True:
            ...
            try:
                response_data = self._call_api(payload, config)
            except requests.exceptions.HTTPError as e:
                self.debug_logger.log_text(f"API call failed: {e}", debug_scope)
                return {"error": {"message": str(e)}}
            except Exception as e:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.use_router:
            # Router will save its state on exit
            self.router.__exit__(exc_type, exc_val, exc_tb)